...
Integrated provider verification remains unfinished. The only authorized paid path is the explicit `HISTORY_UNBROKEN_LIVE_OPENAI_SMOKE=1 OPENAI_API_KEY="$OPENAI_API_KEY" npm run test:live:openai` command. Its separate launcher must validate the opt-in and key without printing either, build with provider credentials, model overrides, and speech secrets explicitly blanked, then start only the live runtime with the supplied key, pinned models, and a fresh ephemeral speech-ticket secret. Ordinary and screenshot Playwright configurations must explicitly blank `OPENAI_API_KEY`, `OPENAI_MODEL`, `OPENAI_SPEECH_MODEL`, and `SPEECH_AUTHORIZATION_SECRET`; regular unit, integration, and Playwright runs remain no-key and deterministic. Do not claim live-provider success until the isolated smoke has actually passed, or claim physical Chromebook performance, classroom efficacy, production privacy compliance, public deployment, or final submission readiness until those checks have actually been completed.

The `high`, `balanced`, and `classroom` graphics profiles are presentation budgets, not historical variants. High and Balanced may use ledgered CC0 PBR facade textures, the ledgered dusk HDRI, limited lantern keys, shadows, contact shadows, and restrained bloom. Classroom must take its low-texture procedural path before rich PBR requests and mounts no HDRI, point lights, contact shadows, or post-processing. Every district zone must report either loaded or deterministic-fallback assets and its canonical interaction readiness; no optional asset or visual effect may block evidence access.
...
- Target ordinary school laptops and Chromebooks. The grounded district must preserve the asset, loading, optional-asset-fallback, nonblank-canvas, and frame-rate gates before any further visual expansion. The latest 2026-07-20 throttled Chromium proxy run failed the archive and warm-traversal frame-rate gates, so do not describe the performance gate as passing or make the spatial route the classroom default pending a clean rerun and physical Chromebook verification.
