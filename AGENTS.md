...
Phase 0, the deterministic primer-to-debrief vertical slice, the source-bounded GPT-5.6 layer, the four-zone 3D investigation through the Louis station, push-to-talk transcription, authorized speech playback, and bounded teacher alignment are implemented in the working tree. AI contract `1.1.0`, media contract `1.0.0`, and course-alignment contract `1.1.0` are active. The current gate is integrated hardening: complete media cleanup and race-condition checks, prove the teacher-to-student path, refresh the controlling documentation, and rerun the complete no-key verification suite before starting the next spatial task.
